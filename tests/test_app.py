...
def test_get_nonexistent_user_by_id():
    # Предполагаем, что пользователя с id 999 нет
...
def test_delete_nonexistent_user():
    # Имя пользователя, которого нет
    response = client.delete('/api/users/non_existent_user')
    assert response.status_code == 404  # Ожидаем, что код 404 - это не найдено
    assert 'detail' in response.json()
...
def test_create_project(setup_db_for_test_projects):
    ...
    # Предполагаем, что код 201 - это успешное создание
    assert response.status_code == 200
...
def test_get_project_by_id():
    ...
    assert response.status_code == 200
    # Проверяем, что получаем правильный проект
    assert response.json()['id'] == 1


def test_get_nonexistent_project_by_id():
    # Предполагаем, что проекта с id 999 нет
...
def test_update_project():
    ...
    # Предполагаем, что код 200 - это успешное обновление
    assert response.status_code == 200
...
def test_delete_nonexistent_project():
    # Имя проекта, которого нет
    response = client.delete('/api/projects/non_existent_project')
    assert response.status_code == 404  # Ожидаем, что код 404 - это не найдено
    assert 'detail' in response.json()
...
def test_get_nonexistent_task():
    # Предполагаем, что задача с id 999 не существует
...
def test_get_comment_by_id():
    ...
    assert response.status_code == 200  # Ожидаем успешный ответ
    # Проверяем, что получен правильный комментарий
    assert response.json()['id'] == 1
...
def test_get_nonexistent_comment():
    # Предполагаем, что комментарий с id 999 не существует
...
def test_get_nonexistent_project_members():
    # Предполагаем, что проект с id 999 не существует
    response = client.get('/api/project_members/project/999')
...
def test_delete_nonexistent_project_member():
    # Предполагаем, что участник с id 999 не существует
    response = client.delete('/api/project_members/999/4')
    assert response.status_code == 404  # Ожидаем, что участник не найден
...
def test_register_user(setup_db_for_test_auth):
    ...
    # Проверяем, что статус-код ответа 200 (или 201, в зависимости от вашей
    # реализации)
    assert response.status_code == 200  # или 201, если вы возвращаете статус создания
    # Измените на ожидаемое сообщение
...
def test_get_task_distribution(setup_db_for_test_buisnes_logic):
    ...
    response = client.get('/api/auth/task-distribution',
    ...
    # Здесь вы можете добавить дополнительные проверки для содержимого
    # distribution
...
def test_apply_distribution():
    ...
    assert response.status_code == 200
    # Измените на ожидаемое сообщение
    assert response.json() == {'message': 'Distribution applied successfully'}
...
def test_get_ranked_tasks():
    ...
    response = client.get('/api/auth/ranked-tasks',
                          headers={'Authorization': f"Bearer {TEST_TOKEN}"})
...
def test_get_ranked_projects():
    ...
    response = client.get('/api/auth/ranked-projects',
                          headers={'Authorization': f"Bearer {TEST_TOKEN}"})
